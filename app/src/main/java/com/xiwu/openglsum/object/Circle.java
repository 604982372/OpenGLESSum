...
public class Circle {
    ...
    float r = 0.8f;
    float color[] = new float[]{0.5f, 0f, 0.5f};//圆的颜色
    private int count;
    ...
    public void initVertexData()//初始化顶点数据的方法
    {
        ...
        alVertix.add(0f);
        alVertix.add(0f);
        for (int angle = 0; angle <= 360; angle = angle + angleSpan) {
            alVertix.add((float) (r * UNIT_SIZE * Math.cos(Math.toRadians(angle))));
            alVertix.add((float) (r * UNIT_SIZE * Math.sin(Math.toRadians(angle))));
            alVertix.add(0f);
        }
        ...
        count = 0;
        float colors[] = new float[vCount * 4];//顶点颜色值数组，每个顶点4 个色彩值RGBA
...
//创建顶点着色数据缓冲
        ...
        mColorBuffer = cbb.asFloatBuffer(); //转换为 Float 型缓冲
